...
import { asyncScheduler, BehaviorSubject, defer, MonoTypeOperatorFunction, Observable, of, Subject, Subscription } from 'rxjs';
...
export abstract class PlatformComponent implements OnInit, AfterViewInit, OnDestroy, OnChanges {
	...
	public ngAfterViewInit(): void {
		...
		// Handle for case parent input ngTemplate for child onPush component. Child activate change detection on init, then parent init ngTemplate
		// view later but template rendered inside child component => need to trigger change detection again for the template from parent to render
		this.detectChanges();
	}
	...
	public observerLoadingErrorState<T>(
		...
		const setLoadingState = () => {
			...
			if (this.isForSetReloadingState(options)) {
	...
	/**
	 * Returns an Signal that emits the error message associated with the specified request key or the first existing error message if requestKey is
	 * default key if error message with default key is null.
	...
	/**
	 * Returns the error message associated with the specified request key or the first existing error message if requestKey is default key if error
	 * message with default key is null.
	 * * @param [requestKey=requestStateDefaultKey] (optional): A key to identify the request. Default is
	 * requestStateDefaultKey.
	 */
